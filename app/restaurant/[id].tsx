...
import { createTableBooking } from "@/api/bookingApi";
import { detectAllergens, isIngredientAllergen } from "@/services/preferencesService";
...
export default function RestaurantPage() {
  ...
  const fetchEverything = async () => {
    ...
    try {
      ...
      showGlobalToast({
        message:
          "Failed to load restaurant data. Please check your connection.",
        type: "error",
      });
    } finally {
      setLoading(false);
  ...
  const hasAllergen = (item: MenuItem): boolean => {
    // Check main item allergens tags
    const directTagMatch = item.allergens.some((a) =>
      preferences.allergies.some((ua) => ua.toLowerCase() === a.toLowerCase()),
    );
    if (directTagMatch) return true;

    // Check detailed ingredients
    const { hasAllergen: hasIngAllergen } = detectAllergens(preferences.allergies, item.ingredients);
    return hasIngAllergen;
  };
  ...
  const handleAddToCart = (item: MenuItem) => {
    ...
    showGlobalToast({ message: `${item.name} added to cart`, type: "success" });
  };
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
              ...
              <View key={review.id} style={styles.reviewMiniCard}>
                <View style={styles.reviewHeader}>
                  <View style={styles.avatarMini}>
                    <Text style={styles.avatarTxtMini}>
                      {String(review.user_name).charAt(0)}
                    </Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.miniReviewName}>
                      {review.user_name}
                    </Text>
                    {renderStars(review.rating)}
                  </View>
                </View>
                <Text style={styles.miniReviewComment} numberOfLines={2}>
                  {review.comment}
                </Text>
              </View>
            ))}
        ...
        {showOffersOverlay && (
          <View style={styles.sectionContainer}>
            ...
            {offers.length > 0 ? (
              offers.map((offer) => (
                <View key={offer.id} style={styles.offerMiniCard}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.offerMiniMeta}>
                      {offer.discount_percent}% OFF • Until{" "}
              ...
              <Text style={styles.emptyTxtMini}>
                No active offers for this restaurant.
              </Text>
            )}
          </View>
        )}
              ...
              >
                <View style={styles.dishImageWrap}>
                  ...
                  {itemQty > 0 && (
                    <View style={styles.dishQtyBadge}>
                      <Text style={styles.dishQtyText}>{itemQty}</Text>
                    </View>
                  )}
                  {isAllergen && (
                ...
                <View style={styles.dishInfo}>
                  ...
                  <TouchableOpacity
                    style={[
                      styles.dishAddBtn,
                      isAllergen && { backgroundColor: "#EF4444" },
                    ]}
                    onPress={(e) => {
        ...
        <View style={styles.modalBg}>
          <View style={styles.modalContent}>
            ...
            <Text style={styles.modalSub}>
              Book your premium table at {restaurant.name}
            </Text>
            ...
            <View style={styles.slotContainer}>
              {[
                ...
                <TouchableOpacity
                  ...
                  style={[
                    styles.slotBtn,
                    selectedSlot === t && styles.slotBtnActive,
                  ]}
                ...
                >
                  <Text
                    style={[
                      styles.slotText,
                      selectedSlot === t && styles.slotTextActive,
                    ]}
                  >
                    {t}
                  </Text>
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                if (!selectedSlot || !guests) {
                  showGlobalToast({
                    message: "Please select time and guests",
                    type: "error",
                  });
                  return;
                }
                ...
                try {
                  ...
                  showGlobalToast({
                    message: `Table booked for ${guests} guests at ${selectedSlot}`,
                    type: "success",
                  });
                  setBookingVisible(false);
                } catch (e: any) {
                  showGlobalToast({
                    message: e.message || "Failed to confirm booking",
                    type: "error",
                  });
                } finally {
      ...
      <Modal
        animationType="slide"
        transparent
        visible={isDishDetailsVisible && !!selectedDish}
      >
        <View style={styles.modalBg}>
          <TouchableOpacity
            activeOpacity={1}
            style={{ flex: 1, width: "100%" }}
            onPress={() => setDishDetailsVisible(false)}
          />
          <View style={styles.dishDetailsCardFull}>
            <View style={styles.dishDetailsImgWrapFull}>
              <Image
                ...
                style={styles.dishDetailsImg}
              />
              <TouchableOpacity
                style={styles.closeDishBtn}
                onPress={() => setDishDetailsVisible(false)}
              >
            ...
            >
              <View style={styles.dishDetailsHeader}>
                <View style={{ flex: 1, paddingRight: 10 }}>
                  <Text style={styles.dishDetailsName}>
                    {selectedDish?.name}
                  </Text>
                  <View style={styles.dishDetailsSubHeader}>
                    ...
                    <View style={styles.badgePillSecondary}>
                      <Text style={styles.badgePillTextSecondary}>
                        For {selectedDish?.recommended_for || "Lunch & Dinner"}
                      </Text>
                    </View>
                  </View>
                </View>
                <Text style={styles.dishDetailsPrice}>
                  ₹{selectedDish?.price}
                </Text>
              </View>
              ...
              <Text style={styles.dishDetailsDescription}>
                {selectedDish?.description ||
                  "A delicious dish carefully prepared with fresh ingredients to satisfy your cravings."}
              </Text>

              <View style={styles.dishDetailsStats}>
                <View style={styles.dishStatBox}>
                  <Ionicons name="flame-outline" size={20} color="#FF7A00" />
                ...
                <View style={styles.dishStatBox}>
                  ...
                  <Text style={styles.dishStatValue}>
                    {selectedDish?.cooking_method
                      ? selectedDish.cooking_method.split("-")[0].trim()
                      : "Classic"}
                  </Text>
                </View>
              </View>
              ...
              {selectedDish?.cooking_method &&
                selectedDish.cooking_method.includes("-") && (
                  <View style={styles.detailsSection}>
                    <Text style={styles.detailsSectionTitle}>
                      Cooking Method
                    </Text>
                    <Text style={styles.detailsSectionText}>
                      {selectedDish.cooking_method.trim()}
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.ingredientList}>
                  ...
                  selectedDish.ingredients.length === 0 ? (
                    ...
                    selectedDish.ingredients.map((ing, i) => (
                      <View key={i} style={styles.ingredientBulletRow}>
                        <View style={styles.bulletPoint} />
                        <View style={{ flexDirection: 'row', alignItems: 'center', flexWrap: 'wrap', flex: 1, gap: 6 }}>
                          <Text style={{ fontWeight: "700", color: "#181C2E", fontSize: 15 }}>
                          ...
                          <View style={[
                            ...
                            isIngredientAllergen(ing.ingredient?.name || "", preferences.allergies) && {
              ...
              >
                ...
                <Text style={styles.dishAddFullText}>
...
const styles = StyleSheet.create({
  ...
  heroMapBtn: {
    ...
    height: 40,
    borderRadius: 20,
    backgroundColor: "rgba(24,28,46,0.6)",
    alignItems: "center",
    justifyContent: "center",
  },
  ...
  vDivider: { width: 1, height: 40, backgroundColor: "#E0E5ED" },
  ...
  detailsSectionText: { fontSize: 15, color: "#6B7280", lineHeight: 24 },
  ...
  badgePillText: { fontSize: 12, fontWeight: "700", color: "#FFF" },
  badgePillSecondary: {
    backgroundColor: "#F0F5FA",
    paddingHorizontal: 12,
    paddingVertical: 4,
  ...
  badgePillTextSecondary: { fontSize: 12, fontWeight: "700", color: "#6B7280" },

  ingredientList: { flexDirection: "column", gap: 10, marginTop: 4 },
  ingredientBulletRow: { flexDirection: "row", alignItems: "center" },
  bulletPoint: {
    width: 6,
    height: 6,
    borderRadius: 3,
